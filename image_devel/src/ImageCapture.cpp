...
bool ImageCapture::init_device(char **dev_name_p, int *fd_p, struct ImageCapture_buffer **buffers_p, unsigned int *n_buffers_p, const char *side_label)
{
  ...
  if (-1 == stat (*dev_name_p, &st)) {
    cerr<< "Cannot identify " << side_label << "side " << *dev_name_p << ": " << errno << strerror (errno)<< endl;
    return false;
  }
  
  if (!S_ISCHR (st.st_mode)) {
    cerr<< "Side " << side_label << *dev_name_p << "is no device" <<endl;
    return false;
  }
  ...
  *fd_p = open (*dev_name_p, O_RDWR /* required */ | O_NONBLOCK, 0);
  ...
  if (-1 == *fd_p) {
   cerr<< "Cannot open " << side_label << "side " << *dev_name_p << ": " << errno << strerror (errno)<< endl;
    return false;
  }
  ...
  if (-1 == xioctl (*fd_p, VIDIOC_QUERYCAP, &cap)) {
    if (EINVAL == errno) {
      cerr<< "Side " << side_label << *dev_name_p << "is no V4L2 device" << endl;
      return false;
    } else {
      cerr << "Other 'QueryCap' error" << endl;
      return false;
    }
  }
  
  if (!(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE)) {
    cerr << "Side " << side_label << *dev_name_p << "is no video capture device" << endl;
    return false;
  }
  
  if (!(cap.capabilities & V4L2_CAP_STREAMING)) {
    cerr << "Side " << side_label << *dev_name_p << "does not support streaming i/o" << endl;
    return false;
  }
  ...
  if (-1 == xioctl (*fd_p, VIDIOC_S_INPUT, &index)) {
    ...
    default:
      cerr << "In method init_device(): No video inputs to select" << endl;
      return false;
    }
  }
  ...
  if (-1 == xioctl (*fd_p, VIDIOC_S_FMT, &fmt)){
    cerr << "In method init_device(): Error setting data format" << endl;
    return false;
  }
  ...
  if (640 != fmt.fmt.pix.width || 480 != fmt.fmt.pix.height) 
    {
      cerr <<"Side " << side_label << "format inconsistent size" << endl;
      return false;
    }
  ...
  if (-1 == xioctl (*fd_p, VIDIOC_REQBUFS, &req)) {
    if (EINVAL == errno) {
      cerr << "Side " << side_label << *dev_name_p << " does not support memory mapping" << endl;
      return false; 
    } 
    else {
      cerr << "In method init_device(): Unable to initiate memory mapped buffer I/O" << endl;
      return false;
    }
  }
  
  if (req.count < 2) {
    cerr << "Insufficient buffer memory on side  " << side_label << *dev_name_p << endl;
    return false;
  }
  ...
  *buffers_p = (ImageCapture_buffer *)calloc (req.count, sizeof (**buffers_p));
  ...
  if (!*buffers_p) {
    cerr << "Side " << side_label << ": Out of memory" << endl;
    return false;
  }
  ...
  for (*n_buffers_p = 0; *n_buffers_p < req.count; ++*n_buffers_p) {
    ...
    if (-1 == xioctl (*fd_p, VIDIOC_QUERYBUF, &buf)){
      cerr << "Issue with buffer querying in VIDIOC_QUERYBUF" <<endl;
      return false;
    }
...
bool ImageCapture::wait_for_device(int *fd_p, const char *side_label)
{
  for(;;) {
    ...
    if (0 == r) {
      cerr << "Side " << side_label << " select timeout" << endl;
      return false;
    }
...
//cn/im: adds a file descriptor to a file descriptor set
  ...
  if (-1 == r) {
    ...
    if (0 == r) {
      cerr<< "Master select timeout" << endl;
      exit (EXIT_FAILURE);
    }
...
Image ImageCapture::writeImage() //writeFrame?
{
	...
	rightfile.close();
	cout << "image save" << endl; //added for trouble shooting
...
bool ImageCapture::good() // returns true if works, false if connections unestablished
{
  ...
  if (-1 == r) {
    if (EINTR == errno)
    cerr << "In method good(): Unreadable device" <<endl;
    return false;
  }
  
  if (0 == r) {
    cerr << "In method good(): Device timed out" << endl;
    return false;
  }

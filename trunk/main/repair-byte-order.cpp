// swapgb.cpp
// reads a ppm file into an Image object and swaps the green and blue channels
...
#pragma omp for
  ...
  for(size_t i = 0; i < bound; i++)
	...
	continue;
      ...
      if(flipByteOrder_gb(*img) != 0) {
	cerr << "Error flipping byte order for image " << images[i] << endl;
      }
      else {
	// print the image out
...
void help() {
  ...
  cerr << "repair-byte-order: flips g and b bytes of given images" << endl;
  cerr << "usage: repair-byte-order [-p path] [--pano panofile]" << endl;
  cerr << "" << endl;  
}

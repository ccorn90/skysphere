...
VISCA_API uint32_t libviscaBRC300::set_dzoom_off(VISCAInterface_t* iface, VISCACamera_t* camera)
{
  VISCAPacket_t packet;
...
char libviscaBRC300::iris_lookup_table(double f) // pick an f-stop
{
  // from Sony BRC300 command list, pg 19
  if( f <= 1.6 ) return 0x11;
...
// manually set the shutter speed -- 0x0x = 1/3 sec, 0x06 = normal, 0x15 = fastest
VISCA_API uint32_t libviscaBRC300::set_absolute_shutter(VISCAInterface_t* iface, VISCACamera_t* camera, char value)
{
  // set end stops
  ...
  _VISCA_append_byte(&packet, VISCA_COMMAND);
  _VISCA_append_byte(&packet, 0x04);
  _VISCA_append_byte(&packet, 0x4A); // set to A for shutter
  _VISCA_append_byte(&packet, 0x00);
  _VISCA_append_byte(&packet, 0x00);
...
VISCA_API uint32_t libviscaBRC300::set_absolute_iris(VISCAInterface_t* iface, VISCACamera_t* camera, char value)
{
  // set end stops
  if(value > 0x11) value = 0x11;
  ...
  _VISCA_append_byte(&packet, VISCA_COMMAND);
  _VISCA_append_byte(&packet, 0x04);
  _VISCA_append_byte(&packet, 0x4B); // set to B for iris
  _VISCA_append_byte(&packet, 0x00);
  _VISCA_append_byte(&packet, 0x00);
  _VISCA_append_byte(&packet, ((value & 0xF0) >> 4) );
  _VISCA_append_byte(&packet, ((value & 0x0F)     ) );
...
VISCA_API uint32_t libviscaBRC300::set_absolute_gain(VISCAInterface_t* iface, VISCACamera_t* camera, char value)
{
  // set end stops
  ...
  _VISCA_append_byte(&packet, VISCA_COMMAND);
  _VISCA_append_byte(&packet, 0x04);
  _VISCA_append_byte(&packet, 0x4C); // set to C for gain
  _VISCA_append_byte(&packet, 0x00);
  _VISCA_append_byte(&packet, 0x00);
  _VISCA_append_byte(&packet, ((value & 0xF0) >> 4) );
  _VISCA_append_byte(&packet, ((value & 0x0F)     ) );
...
VISCA_API uint32_t libviscaBRC300::set_auto_exposure(VISCAInterface_t* iface, VISCACamera_t* camera)
{
  VISCAPacket_t packet;
...
VISCA_API uint32_t libviscaBRC300::set_manual_exposure(VISCAInterface_t* iface, VISCACamera_t* camera)
{
  VISCAPacket_t packet;

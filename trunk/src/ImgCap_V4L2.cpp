...
bool ImgCap_V4L2::init_device(char **dev_name_p, int *fd_p, struct ImgCap_V4L2_buffer **buffers_p, unsigned int *n_buffers_p, const char *side_label)
{
 ...
 /*Copied from open_device - ATP12*/
  ...
  if (-1 == stat (*dev_name_p, &st)) {
    if(ImgCap_V4L2::verboseErrorPrinting) std::cerr << "Cannot identify " << side_label << "side " << *dev_name_p << ": " << errno << strerror (errno)<< std::endl;
    return false;
  }
  
  if (!S_ISCHR (st.st_mode)) {
    if(ImgCap_V4L2::verboseErrorPrinting) std::cerr << "Side " << side_label << *dev_name_p << "is no device" <<std::endl;
    return false;
  }
  ...
  *fd_p = open (*dev_name_p, O_RDWR /* required */ | O_NONBLOCK, 0);
  ...
  if (-1 == *fd_p) {
    if(ImgCap_V4L2::verboseErrorPrinting) std::cerr << "Cannot open " << side_label << "side " << *dev_name_p << ": " << errno << strerror (errno)<< std::endl;
    return false;
  ...
  if (-1 == xioctl (*fd_p, VIDIOC_QUERYCAP, &cap)) {
    if (EINVAL == errno) {
      if(ImgCap_V4L2::verboseErrorPrinting) std::cerr << "Side " << side_label << *dev_name_p << "is no V4L2 device" << std::endl;
      return false;
    } else {
  ...
  if (!(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE)) {
    if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  << "Side " << side_label << *dev_name_p << "is no video capture device" << std::endl;
    return false;
  }
  
  if (!(cap.capabilities & V4L2_CAP_STREAMING)) {
    if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  << "Side " << side_label << *dev_name_p << "does not support streaming i/o" << std::endl;
    return false;
  }
  ...
  if (-1 == xioctl (*fd_p, VIDIOC_S_INPUT, &index)) {
    ...
    default:
      if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  << "In method init_device(): No video inputs to select" << std::endl;
      return false;
    }
  }
  ...
  if (-1 == xioctl (*fd_p, VIDIOC_S_FMT, &fmt)){
    if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  << "In method init_device(): Error setting data format" << std::endl;
    return false;
  }
  ...
  if (640 != fmt.fmt.pix.width || 480 != fmt.fmt.pix.height) 
    {
      if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  <<"Side " << side_label << "format inconsistent size" << std::endl;
      return false;
    }
  ...
  if (-1 == xioctl (*fd_p, VIDIOC_REQBUFS, &req)) {
    if (EINVAL == errno) {
      if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  << "Side " << side_label << *dev_name_p << " does not support memory mapping" << std::endl;
      return false; 
    } 
    else {
      if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  << "In method init_device(): Unable to initiate memory mapped buffer I/O" << std::endl;
      return false;
    }
  }
  
  if (req.count < 2) {
    if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  << "Insufficient buffer memory on side  " << side_label << *dev_name_p << std::endl;
    return false;
  }
  
  *buffers_p = (ImgCap_V4L2_buffer *)calloc (req.count, sizeof (**buffers_p));
  
  if (!*buffers_p) {
    if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  << "Side " << side_label << ": Out of memory" << std::endl;
    return false;
  }
  ...
  for (*n_buffers_p = 0; *n_buffers_p < req.count; ++*n_buffers_p) {
    ...
    if (MAP_FAILED == (*buffers_p)[*n_buffers_p].start){
      if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  << "mmap" <<std::endl;
      return false;
    } 
  }
  return true;
...
bool ImgCap_V4L2::start_capturing(int *fd_p, unsigned int *n_buffers_p)
{
  /* fd, n_buffers, label */
  unsigned int i;
  ...
  for (i = 0; i < *n_buffers_p; ++i) {
    ...
    if (-1 == xioctl (*fd_p, VIDIOC_QBUF, &buf)){
      if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  << "VIDIOC_QBUF" <<std::endl;
      return false;
    }
  }
  ...
  if (-1 == xioctl (*fd_p, VIDIOC_STREAMON, &type)){
    if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  << "VIDIOC_STREAMON" <<std::endl;
    return false;
    }
  return true;
}
...
void ImgCap_V4L2::halt_capturing(int *fd_p, struct ImgCap_V4L2_buffer **buffers_p)
{
  enum v4l2_buf_type type;
  unsigned int i;
...
bool ImgCap_V4L2::wait_for_device(int *fd_p, const char *side_label)
{
  for(;;) {
    fd_set fds;
    struct timeval tv;
...
int ImgCap_V4L2::read_frame(int *fd_p, struct v4l2_buffer *buf_p)
{
  sleep(.1); //added so that the video card can stablize
  IMGCAP_V4L2_CLEAR (*buf_p);
...
void ImgCap_V4L2::save_frames() // optional - call to capture images (i.e. for calibration)
{
...
void ImgCap_V4L2::save_frames(const char * fileName) //for saving with a given file name
{
  //moves memory via C methods
        memcpy(sds_bufferR+1, r_buffers[r_buf.index].start, SINGLE_FRAME_SIZE);
...
// create an ImgCap_V4L2 object with a devicename
// device names are:/dev/video0 or /dev/video1 and the typical deviceIndex is:2 
ImgCap_V4L2::ImgCap_V4L2(const char* device, int deviceIndex) : ImgCap_base(device, deviceIndex, ImgCap_base::V4L2) {
  // TODO: other inits!
  quit_signal = 0;
  r_fd = -1;
  r_buffers = NULL;
...
// create an ImgCap_V4L2 object, with the first constructor, and a device index of "2" representing S-Video.
ImgCap_V4L2::ImgCap_V4L2(const char* device): ImgCap_base(device, 2, ImgCap_base::V4L2) { 
  quit_signal = 0;
  r_fd = -1;
  r_buffers = NULL;
  r_n_buffers = 0;
...
// destruct the ImgCap_V4L2 object
ImgCap_V4L2::~ImgCap_V4L2() { }
...
//tear all down
bool ImgCap_V4L2::closeConnection() {
  if (true == cameraOn){
...
bool ImgCap_V4L2::good() // returns true if works, false if connections unestablished
{
  fd_set fds;
  ...
  if (-1 == r) {
    if (EINTR == errno)
      if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  << "In method good(): Unreadable device" << std::endl;
    return false;
  }
  
  if (0 == r) {
    if(ImgCap_V4L2::verboseErrorPrinting) std::cerr  << "In method good(): Device timed out" << std::endl;
    return false;
  }

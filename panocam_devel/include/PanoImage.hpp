...
using std::stringstream;  using std::fstream;

// for openMP parallel flags -- def-ne this variable with -D when you compile
...
public:
  ...
  PixelLoc findPixelLocFromCenter (double thetaPrime, double phiPrime, double zoom);// theta and phi from centerof the element, not total!
...
public:
  ...
  // Extraction of data
  enum blendMode { SELECT, AVERAGE, WEIGHTED };  // TODO: add more options
